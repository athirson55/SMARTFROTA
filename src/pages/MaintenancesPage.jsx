...
export function MaintenancesPage() {
  ...
  const counters = useMemo(() => {
    const total = maintenancesSeed.length;
    const pendente = maintenancesSeed.filter(
      (item) => item.status === "pendente",
    ).length;
    const andamento = maintenancesSeed.filter(
      (item) => item.status === "em_andamento",
    ).length;
    const concluida = maintenancesSeed.filter(
      (item) => item.status === "concluida",
    ).length;
    const preventiva = maintenancesSeed.filter(
      (item) => item.type === "preventiva",
    ).length;
    const corretiva = maintenancesSeed.filter(
  ...
  const filtered = useMemo(() => {
    ...
    const list = maintenancesSeed.filter((item) => {
      const filterMatch =
        activeFilter === "todos" ||
